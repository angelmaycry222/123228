...
async function fetchRecentBuys(wallet) {
  ...
  try {
    ...
    return data
      .filter(tx => tx.type === 'SWAP' && tx.swapChanges && tx.swapChanges.after)
      .map(tx => tx.swapChanges.after.mint) || [];
  } catch (err) {
    console.error(`Error fetching tx for ${wallet}:`, err.response?.status || err.message);
    return [];
  }
}
...
async function checkForMatches() {
  ...
  for (const wallet of WATCHED_WALLETS) {
    ...
    console.log(`Wallet ${wallet} recent buys:`, Array.from(unique)); // Log recent buys for debugging
  ...
  for (const [mint, count] of Object.entries(buyCounts)) {
    if (count >= 2 && !notifiedMints.has(mint)) {
      ...
      bot.sendMessage(CHAT_ID, `🚨 ${count} wallets bought token: ${mint}`)
